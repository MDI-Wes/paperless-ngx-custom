...
FROM ghcr.io/astral-sh/uv:0.9.18-python3.12-bookworm-slim AS s6-overlay-base
...
RUN --mount=type=cache,target=${UV_CACHE_DIR},id=python-cache \
  ...
  && echo "Installing Python requirements" \
    ...
    && uv pip install --system --no-python-downloads --python-preference system --no-verify-hashes --requirements requirements.txt \
